...
namespace MFramework
{
    ...
    public class LoadUIHintWindows : SingletonByMono<LoadUIHintWindows>
    {
        ...
        ///UI预设 根目录
        /// </summary>
        public static string hintBorderRootPath = "Assets/" + ABSetting.assetsSubPath + "/Prefab/HintBorder";
        /// <summary>
        /// 无按钮
        /// </summary>
        ...
        private static GameObject CreateHintWindows(string hintBorderRootPath, string hintSubPath)
        {
            //TODO 后期优化 使用对象池
            //生成提示框
            Debug.Log("TAB " + hintBorderRootPath + "/" + hintSubPath + ".prefab");
            GameObject clone = LoadResource.LoadSync<GameObject>(hintBorderRootPath + "/" + hintSubPath + ".prefab", ResType.Null, true);
            if (clone != null)
            {
                //配置窗体位置、大小信息
                ...
                if (cloneParent == null)
                {
                    Debug.Log("TAB " + hintBorderRootPath + "/cvsUIHint.prefab");
                    GameObject imgHintSimple = LoadResource.LoadSync<GameObject>(hintBorderRootPath + "/cvsUIHint.prefab", ResType.Null, true);
