...
namespace MFramework
{
    ...
    public class BuildTool : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        {
            Debugger.Log("Building Start...  StartTime:" + report.summary.buildStartedAt, LogTag.MF);
        }

        public void OnPostprocessBuild(BuildReport report)
        {
            Debugger.Log("Building End" +
        ...
        private static void Build(string outPutFolderPath)
        {
            ...
            {
                Debugger.LogError("打包失败！打包路径为空，请使用编辑器工具配置打包路径 " + EditorMenuRootPath + "/BuildConfigSetting");
                return;
            }
        ...
        {
            Debugger.Log("Building......", LogTag.MF);
            SetConfigSetting();
        ...
        public static void SetConfigSetting()
        {
            ...
            Debugger.Log("------------------------------------------------------------解析CMD参数------------------------------------------------------------", LogTag.MF);
            foreach (var s in args)
            {
                Debugger.Log("参数遍历 arg:" + s, LogTag.MF);
                if (s.Contains("--productName:"))
                {
                    Debugger.Log("--productName:" + s, LogTag.MF);
                ...
                {
                    Debugger.Log("--version:" + s, LogTag.MF);
                    string version = s.Split(':')[1];
                    // 设置版本号
                    PlayerSettings.bundleVersion = version;
                    foreach (var item in s.Split(':'))
                    {
                        Debugger.Log("BBB " + item, LogTag.MF);
                    }
                }
                if (s.Contains("--outPutPath:"))
                {
                    Debugger.Log("--outPutPath:" + s, LogTag.MF);
        ...
        private static string[] FindEnableEditorrScenes()
        {
            ...
            foreach (EditorBuildSettingsScene scene in EditorBuildSettings.scenes)
            {
                ...
                Debugger.Log(scene.path, LogTag.MF);
            }
        ...
        public static void OpenDirectory(string path)
        {
            ...
            {
                Debugger.LogError("No Directory: " + path);
                return;
            }
